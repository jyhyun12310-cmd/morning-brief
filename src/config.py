...
# 시가총액 하한 — 이보다 작으면 후보에서 제외.
# 80억 달러면 S&P 500 하위권~중견 기업 수준이라 이름이 어느 정도 알려져 있고
# 애널리스트 커버리지도 붙습니다. 무명 소형주(수십억 달러)는 걸러지면서도
# 후보가 충분히 남아 매일 같은 대형주만 반복되지 않습니다.
MIN_MARKET_CAP = 8e9       # 80억 달러
# 주가 하한 — 페니스톡 제외
MIN_PRICE = 10.0
# 하루 최소 거래대금 — 이 정도는 돌아야 기관이 실제로 참여한 움직임입니다
MIN_DOLLAR_VOLUME = 1.5e8  # 1.5억 달러

# 하루 등락률 상한. 대형주가 이 이상 움직이면 보통 데이터 오류이거나
# 합병·분할 같은 특수 이벤트라 일반적인 시황 해설에 맞지 않습니다.
MAX_ABS_PCT = 25.0

# 시총 구간별 가중치. 같은 폭으로 움직여도 큰 회사를 우선합니다.
CAP_WEIGHTS = [
    (5e11, 1.45),   # 5000억 달러 이상 (애플·엔비디아급)
    (2e11, 1.30),   # 2000억 이상
    (1e11, 1.15),   # 1000억 이상
    (0,    1.00),   # 그 미만 (하한 300억)
]
...
# 거래량 급증은 중요한 신호지만, 뉴스가 붙은 종목보다 우선하면 안 됩니다.
# (거래량만 터지고 이유를 모르는 종목은 해설할 내용이 없어 카드가 부실해집니다)
RVOL_CAP = 4.0
RVOL_WEIGHT = 0.28

# 뉴스에 언급된 종목 가산. 해설할 스토리가 있다는 뜻이라 강하게 우대합니다.
NEWS_MULT = 2.2
